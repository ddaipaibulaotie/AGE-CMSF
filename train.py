...
import random
import logging
from merge_tokens import get_entity_visual_tokens, get_entity_textual_tokens, get_entity_textual_tokens_mdkg
...
if __name__ == "__main__":
    ...
    parser.add_argument('--score_function', default="tuckerEa", type=str)
    ...
    log_name = f"{args.exp}_{args.data}_{file_format}.log"
    ...
    visual_token_index = visual_token_index.to(torch.device('cpu'))
    ...
    text_token_index = text_token_index.to(torch.device('cpu'))
    # text_token_index = text_token_index.cuda()
    logger.info(f"Visual token index: {visual_token_index}, Text token index: {text_token_index}")
    logger.info(f"Visual key mask: {visual_key_mask}, Text key mask: {text_key_mask}")
    ...
    all_ents = torch.arange(KG.num_ent).to(torch.device('cpu'))
    # all_ents = torch.arange(KG.num_ent).cuda()
    all_rels = torch.arange(KG.num_rel).to(torch.device('cpu'))
    ...
    for epoch in range(last_epoch + 1, args.num_epoch + 1):
        ...
        for batch, label in tqdm(KG_Loader):
            ...
            loss += loss_b
            scores = model.score(ent_embs, rel_embs, batch.to(torch.device('cpu')))
            # scores = model.score(ent_embs, rel_embs, batch.cuda())
            loss_f = loss_fn(scores, label.to(torch.device('cpu')))
        ...
        if (epoch) % args.valid_epoch == 0:
            ...
            with torch.no_grad():
                ...
                for triplet in tqdm(KG.valid):
                    ...
                    head_score = model.score(ent_embs, rel_embs, torch.tensor([[KG.num_ent + KG.num_rel, r + KG.num_ent, t + KG.num_rel]]).to(torch.device('cpu')))[0].detach().cpu().numpy()
                    ...
                    tail_score = model.score(ent_embs, rel_embs, torch.tensor([[h + KG.num_rel, r + KG.num_ent, KG.num_ent + KG.num_rel]]).to(torch.device('cpu')))[0].detach().cpu().numpy()
